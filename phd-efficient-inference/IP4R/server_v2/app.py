...
_JOBS_DIR    = _REPO_ROOT / "data" / "jobs_v2"
...
# ── FastAPI app ───────────────────────────────────────────────────────────────
app = FastAPI(
    title="FQCT Inference Server v2 — DL+CV Sandwich",
    description="EfficientNet-B0 + structural CV verification (18:88 / 88 / bars / 88888)",
    version="2.1.0",
)
...
<body>
<h1>FQCT Inference Server <span style="font-size:.6em;color:#888">v2 · DL+CV</span></h1>
<sub>EfficientNet-B0 · structural CV verify (18:88 / 88 / ▂▄█ / 88888) · port 8081</sub>
...
function showResult(data) {
  ...
  // DL + CV sub-line
  const dlC = res.dl_verdict==='PASS' ? '#4caf50' : '#ef5350';
  const cvC = res.cv_passed ? '#4caf50' : '#ef5350';
  document.getElementById('r-meta').innerHTML =
    `Job: ${data.job_id} &nbsp;|&nbsp; ` +
    `<span style="color:${dlC}">DL: ${res.dl_verdict??'—'} (${res.median_p2_prob!=null?(res.median_p2_prob*100).toFixed(1)+'%':'—'})</span>` +
    ` &nbsp;|&nbsp; ` +
    `<span style="color:${cvC}">CV: ${res.cv_passed?'PASS':'FAIL'}</span>` +
    ` &nbsp;|&nbsp; P2 frames: ${res.p2_pass??'—'}/${res.p2_frames??'—'}` +
    ` &nbsp;|&nbsp; ${res.inference_ms!=null?res.inference_ms.toFixed(0)+'ms':'—'}`;
  ...
  const dlRows = (res.roi_results || []).filter(r => r.name !== 'icon_strip');
  tbody.innerHTML = '<tr><th colspan="4" style="background:#1a3a5c">DL — Phase-2 Coverage</th></tr>' +
    '<tr><th>Segment</th><th>Coverage</th><th>Min</th><th>Status</th></tr>' +
    dlRows.map(r => `<tr>
      <td>${r.name}</td><td>${r.coverage}</td><td>${r.min_cov}</td>
      <td style="color:${r.passed?'#4caf50':'#ef5350'}">${r.passed?'OK':'FAIL'}</td>
    </tr>`).join('');

  // CV structural verify table
  const cvZones = (res.cv_result||{}).zones || {};
  let cvHTML = '<tr><th colspan="4" style="background:#1a4a2a;margin-top:8px">CV — Structural Verify</th></tr>' +
               '<tr><th>Zone</th><th>Char</th><th>Status</th><th>Details</th></tr>';
  for (const [zoneName, zData] of Object.entries(cvZones)) {
    if (zoneName === 'signal_bars') {
      const ok = zData.passed;
      cvHTML += `<tr>
        <td>signal_bars</td><td>▂▄█</td>
        <td style="color:${ok?'#4caf50':'#ef5350'}">${ok?'OK':'FAIL'}</td>
        <td style="font-size:.75rem">${zData.failures&&zData.failures.length?zData.failures.join(', '):'heights='+JSON.stringify(zData.bar_heights)}</td>
      </tr>`;
    } else {
      for (const ch of (zData.chars||[])) {
        const ok = ch.passed;
        cvHTML += `<tr>
          <td style="font-size:.78rem">${zoneName}</td>
          <td><b>${ch.expected}</b></td>
          <td style="color:${ok?'#4caf50':'#ef5350'}">${ok?'OK':'FAIL'}</td>
          <td style="font-size:.72rem;color:#aaa">${(ch.failures||[]).join(', ')||'—'}</td>
        </tr>`;
      }
    }
  }
  tbody.innerHTML += cvHTML;
}
